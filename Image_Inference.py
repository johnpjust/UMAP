...
def img_samples(filename, args):
    ...
    rows = imgcre.shape[0]-rand_box[0]
    cols = imgcre.shape[1] - rand_box[1]
    im_breakup_array = np.zeros((np.int(rows/args.spacing),np.int(cols/args.spacing),rand_box_size*rand_box_size*3), dtype=np.float32)
    with tf.device(args.device):
        for i in range(np.int(rows/args.spacing)*args.spacing):
            if not i%args.spacing:
                for j in range(np.int(cols/args.spacing)*args.spacing):
                    if not j%args.spacing:
                        im_breakup_array[np.int(i/args.spacing),np.int(j/args.spacing),:] = tf.reshape(tf.image.crop_to_bounding_box(imgcre, i, j, rand_box_size, rand_box_size)/255, [-1]).numpy()
    return im_breakup_array
...
def main():
    ...
    args.residual = 'gated'
    args.expname = ''
    # args.load = r'C:\Users\justjo\PycharmProjects\BNAF_tensorflow_eager\checkpoint\corn_layers1_h12_flows6_resize0.25_boxsize0.1_gated_2019-08-25-22-18-30' ## best corn
    args.load = r'C:\Users\justjo\PycharmProjects\BNAF_tensorflow_eager\checkpoint\corn_layers1_h6_flows3_resize0.25_boxsize0.01_gated_2019-08-28-14-06-58'
    args.save = True
    ...
    args.rand_box = 0.01  ##relative size of random box from image
    ...
    fnames = glob.glob('data/GQ_Images/*.png')
    fnames = glob.glob('data\GQ_Images\Corn_2017_2018/*.png')
    ...
    data_samples = []
    data_samples.extend(img_samples(f, args) for f in fnames)
    datasamples = np.array(data_samples)
    datasamples = np.concatenate(np.concatenate(datasamples))
    ...
    with open('data\GQ_Images\Corn_2017_2018\dfnames.csv', 'w') as f:
        for item in fnames:
            f.write("%s\n" % item)
